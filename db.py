...
def save_segments(job_id: str, segments: list, *, step: Optional[str] = None,
    ...
    the step/title. Used by /segment to seed the proposal and by the frontend
    autosave. Returns False if the project doesn't exist."""
    with _connect() as conn:
        cur = conn.execute(
            """
            UPDATE projects SET
              data = jsonb_set(data, '{segments}', %(segments)s, true),
