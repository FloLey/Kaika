...
| [`specs/compositions/`](specs/compositions/) | The composition-DAG montage rework — **IN PROGRESS**; the README carries the per-step status and the locked decisions |
...
- Whole-clip and block-streaming render paths must stay in lockstep. A new video
  ...
  is exactly `fluid`, `output` and `fire`; every other card
  ...
  scan, which is exactly what the whole-clip path was hand-rolling. `montage`
  left with the extracts rework: its lazy child-Dag dict is created fresh per
  scan too.) `test_card_impact` asserts whole == streamed for every card,
