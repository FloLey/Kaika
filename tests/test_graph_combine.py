...
def _fluid(nid, color, pos, angle):
    ...
    ports = {k: {"binding": {"kind": "const", "value": v}} for k, v in
             [("r", r), ("g", g), ("b", b), ("angle", angle),
              ("force", 42), ("emit", 0.45), ("radius", 0.07)]}
    return {"id": nid, "type": "fluid", "x": 0, "y": 0, "data": {
        "static": {"enabled": True, "radial": False, "wrap": True, "points": [pos],
                   "path_speed": 1, "path_closed": False, "path_pingpong": False,
                   "intensity": 1.0, "opacity": 1.0, "color": list(color)},
        "ports": ports}}


def _edge(s, t, tp):
    return {"id": f"e-{s}-{t}-{tp}", "source": s, "sourcePort": "out", "target": t, "targetPort": tp}


def _combine(mode, op1=1.0):
    return {"id": "cb", "type": "combine", "x": 0, "y": 0, "data": {
        "mode": mode, "inputs": [{"id": "s0", "opacity": 1.0}, {"id": "s1", "opacity": op1}],
...
def test_simulate_sources_list_equals_single_source():
    base = {"grid": 32, "duration": 1, "source": {"force": 20, "color": [0.3, 0.7, 1.0]}, "fluid": {}}
...
def test_merge_differs_from_stack():
    diff = np.abs(_frames(_graph("merge")).astype(int) - _frames(_graph("stack")).astype(int)).mean()
    assert diff > 1.0
...
def test_output_passthrough_renders():
    ...
    cb = _combine("stack")
    g = {"version": 1, "nodes": [fa, fb, cb, o1, o2], "edges": [
        _edge("fA", "o1", "video"), _edge("o1", "cb", "s0"),
        _edge("fB", "cb", "s1"), _edge("cb", "o2", "video")]}
...
def test_stack_feeding_merge_rejected():
    ...
    stack = {"id": "sc", "type": "combine", "x": 0, "y": 0,
             "data": {"mode": "stack", "inputs": [{"id": "a"}], "medium": {}}}
    merge = {"id": "mc", "type": "combine", "x": 0, "y": 0,
             "data": {"mode": "merge", "inputs": [{"id": "b"}], "medium": {}}}
    g = {"version": 1, "nodes": [fa, stack, merge,
         {"id": "out1", "type": "output", "x": 0, "y": 0, "data": {}}],
...
def test_merge_respects_per_component_wrap():
    ...
    def fl(nid, wrap):
        ports = {k: {"binding": {"kind": "const", "value": v}} for k, v in
                 [("force", 55), ("angle", 0), ("emit", 0.5), ("radius", 0.06),
                  ("r", 1), ("g", 1), ("b", 1)]}
        return {"id": nid, "type": "fluid", "x": 0, "y": 0, "data": {
            "static": {"enabled": True, "radial": False, "wrap": wrap, "points": [[0.15, 0.5]],
                       "path_speed": 1, "path_closed": False, "path_pingpong": False,
                       "intensity": 1.0, "opacity": 1.0, "color": [1, 1, 1]}, "ports": ports}}
    cbn = {"id": "cb", "type": "combine", "x": 0, "y": 0,
           "data": {"mode": "merge", "inputs": [{"id": "s0"}, {"id": "s1"}], "medium": {}}}
    out = {"id": "o", "type": "output", "x": 0, "y": 0, "data": {}}

    def mean(a_wrap, b_wrap, order=("A", "B")):
        g = {"version": 1, "nodes": [fl("A", a_wrap), fl("B", b_wrap), cbn, out],
             "edges": [_edge(order[0], "cb", "s0"), _edge(order[1], "cb", "s1"), _edge("cb", "o", "video")]}
