...
def test_wrap_retains_dye_that_open_lets_escape():
    ...
    def total_after_advect(wrap: bool) -> float:
        sim = fluid.FluidSim(8, 8, dissipation=1.0, vel_dissipation=1.0, viscosity=0.0,
                             vorticity=0.0, wrap=wrap)
        sim.dye[0][:] = 0.0
...
def test_simulate_returns_a_bounded_uint8_clip_on_a_tiny_grid():
    params = {
        ...
        "grid": 16,
        "source": {
            "emit": 0.3, "radius": 0.1, "force": 10, "angle": 270, "radial": False,
            "wrap": True, "enabled": True, "color": [0.3, 0.7, 1.0], "intensity": 1.0,
            "opacity": 1.0, "points": [[0.5, 0.5]], "path_speed": 1, "path_closed": False,
            "path_pingpong": False,
        },
        "fluid": {"dissipation": 0.95, "velocity_dissipation": 0.97, "viscosity": 0.0,
                  "vorticity": 5.0},
    }
