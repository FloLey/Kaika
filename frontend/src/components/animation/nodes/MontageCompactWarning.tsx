...
export default function MontageCompactWarning({
  ...
  const dupRows = useMemo(() => {
    // `repeats[i].row` is `first + 1` (raw index), so an extract's own number is `i + 1`.
    const rows = repeats
    ...
    <div className="anim-compact-warns">
      {black && (
        <span
          ...
          title={
            ...
            "A clip with loop off goes BLACK once it runs out — expand the card to fix it."
          }
        >
      ...
      {dupRows.n > 0 && (
        <span
          ...
          title={
            `${dupRows.n} extract${dupRows.n === 1 ? "" : "s"} repeat an earlier clip: ` +
            `${dupRows.rows
              .map(
                (r) =>
              ...
              .join(", ")}. ` +
            "Two extracts of the same footage from the same in-point play frame-identical — it reads as a loop, " +
