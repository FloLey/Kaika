...
export function useGraphEditor(opts: GraphEditorOpts) {
  ...
  const graph = useMemo(() => normalizeGraph((segment.graph as Graph) || emptyGraph()), [segment.graph]);
  ...
  const onConnect = useCallback((srcId: string, srcPort: string, tgtId: string, tgtPort: string) => {
    applyUpdater((g) => {
  ...
  const onEdgeDelete = useCallback((edge: GraphEdge) => {
    applyUpdater((g) => {
  ...
  const onNodeDelete = useCallback((node: GraphNode) => {
  ...
  const ctx: NodeCtx = {
    ...
    onDeleteNode: (id: string) => { applyUpdater((g) => removeNode(g, id)); setSelId(null); },
  };
