...
export const ARG_HELP: Record<string, Record<string, string>> = {
  ...
  lyrics: {
    ...
    sizing:
      "auto = each line fits the box (size varies with line length); fixed = every line renders at the same size; min\u2013max = the fit, clamped between two sizes. All sizes are % of the frame height, so the export matches the preview at any resolution.",
    sizeFixed:
      "The one size every line renders at, as % of the frame height. It wins over the box \u2014 a long line wraps and may overflow it (readable beats unreadable).",
    sizeMin:
    ...
    sizeMax:
      "Cap for the auto-fit, as % of the frame height \u2014 a short line stops growing here instead of filling the whole box (shorter lines still render smaller: cap, not fixed).",
  ...
  montage: {
    ...
    breakpoints:
      "The two cut sources, one strip: GATE cuts (from the wired trigger, recomputed live as you tune the signal) and MANUAL cuts. Click empty space on the rail to place a manual cut; drag one to move it, click one to delete it. Click a gate cut to disable just that one — it stays visible, greyed, so its origin reads; a disabled cut that MOVES (threshold edit) re-enables itself. The coloured lane above is material coverage, one colour per extract (matching the dot on its tile): click a band to select that extract's tile in the strip; the band under the playhead glows — that's the video playing right now. Near-black stretches are where the export will render BLACK (a clip out of material with loop off).",
  },
  scope: {
