...
export const ARG_HELP: Record<string, Record<string, string>> = {
  ...
  montage: {
    ...
    breakpoints:
      "The two cut sources, one strip: GATE cuts (from the wired trigger, recomputed live as you tune the signal) and MANUAL cuts. Click empty space to place a manual cut; drag one to move it, click one to delete it. Click a gate cut to disable just that one — it stays visible, greyed, so its origin reads; a disabled cut that MOVES (threshold edit) re-enables itself. The strip's shading is material coverage: tinted where an extract has footage, near-black where the export will render BLACK (a clip out of material with loop off).",
  },
  scope: {
