...
def build_inpaint(model, controlnet):
    ...
    pipe = StableDiffusionControlNetInpaintPipeline.from_pretrained(
        model, controlnet=cnet, torch_dtype=dt, safety_checker=None).to(device)
...
def density_mask(dye, W, H):
    ...
    g = cv2.cvtColor(cv2.resize(np.asarray(dye), (W, H)), cv2.COLOR_RGB2GRAY).astype(np.float32) / 255.0
...
def run(src_key, prompt, tag, denoise=1.0, control_scale=0.8, frames=120, res=384, seed=1):
    ...
    for t in range(len(src)):
        ...
        t0 = time.time()
        res_img = pipe(prompt=prompt, negative_prompt=P.DEFAULT_NEG,
                       image=black, mask_image=mask, control_image=control,
                       height=H, width=W, strength=float(denoise), num_inference_steps=steps,
                       guidance_scale=0.0, controlnet_conditioning_scale=float(control_scale),
