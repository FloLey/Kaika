...
def cb(pp, i, t, kw):
    lat = kw["latents"]; n = len(pp.scheduler.timesteps)
    if i < n - 1:
...
def render(src_key, prompt, tag, steps=8, control_scale=0.8, frames=60, res=384, seed=1):
    ...
    noise = torch.randn(img_lat.shape, generator=torch.Generator("cpu").manual_seed(seed)).to(device, DT)
    ...
    for t in range(frames):
        ...
        t0 = time.time()
        img = pipe(prompt=prompt, negative_prompt=P.DEFAULT_NEG, image=black, control_image=control,
                   height=H, width=W, strength=1.0, num_inference_steps=steps, guidance_scale=0.0,
                   controlnet_conditioning_scale=control_scale, generator=gen,
    ...
    subprocess.run(["ffmpeg", "-y", "-v", "error", "-f", "rawvideo", "-pix_fmt", "rgb24", "-s",
                    f"{W}x{H}", "-r", "24", "-i", "-", "-an", "-c:v", "libx264", "-crf", "28",
                    "-pix_fmt", "yuv420p", "-movflags", "+faststart", str(p)], input=out.tobytes(), check=True)
    print(f"  [{tag}] DONE {W}x{H} {np.mean(times):.2f}s/f -> {p}", flush=True)


if __name__ == "__main__":
    n = int(sys.argv[1]) if len(sys.argv) > 1 else 60
    render("d0e3b447c35129f5", "molten lava, glowing embers, flowing magma, dark background, cinematic",
           "DIFF_dense_lava", frames=n)
