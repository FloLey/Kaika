...
i2i = ZImageImg2ImgPipeline.from_pretrained(HD, torch_dtype=torch.bfloat16, low_cpu_mem_usage=False).to("mps")
i2i.set_progress_bar_config(disable=True)
inp = ZImageInpaintPipeline(**i2i.components); inp.set_progress_bar_config(disable=True)
t2i = ZImagePipeline(**i2i.components); t2i.set_progress_bar_config(disable=True)
...
def mask_img(dye):
    ...
    return Image.fromarray((np.clip(cv2.GaussianBlur(g, (0, 0), 3) * 3, 0, 1) * 255).astype(np.uint8))

def encode(frames, name):
    subprocess.run(["ffmpeg", "-y", "-v", "error", "-f", "rawvideo", "-pix_fmt", "rgb24", "-s",
        f"{W}x{H}", "-r", "24", "-i", "-", "-an", "-c:v", "libx264", "-crf", "30", "-vf",
        "scale=224:-2", "-pix_fmt", "yuv420p", "-movflags", "+faststart", str(OUT / name)],
        input=frames.tobytes(), check=True)

def render(recipe, fk, pk):
    key, start = FLU[fk]; dyes = frames_of(key, start); prompt = PROMPTS[pk]
    black = Image.fromarray(np.zeros((H, W, 3), np.uint8))
    out = np.empty((FRAMES, H, W, 3), np.uint8); t0 = time.time()
    for i, dye in enumerate(dyes):
        g = torch.Generator("cpu").manual_seed(1)
        if recipe == "mask":
            img = inp(prompt=prompt, image=black, mask_image=mask_img(dye), strength=1.0,
                      num_inference_steps=8, guidance_scale=0.0, generator=g).images[0]
        elif recipe == "fluid":
            img = i2i(prompt=prompt, image=Image.fromarray(dye), strength=0.8,
                      num_inference_steps=8, guidance_scale=0.0, generator=g).images[0]
        else:  # noise (txt2img, fluide-indépendant)
            img = t2i(prompt=prompt, height=H, width=W, num_inference_steps=8,
