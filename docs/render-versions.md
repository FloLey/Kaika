...
| v18 | The streaming encoder (every preview and the whole-song master) drops B-frames (`-bf 0`): their 2-frame reorder delay is a REAL presentation offset in a fragmented mp4 (no edit list — `start_time` 0.083s at 24fps), so every streamed clip's video lagged its audio/transport by two frames, and the export master shipped with the offset baked in (video ~67ms late at 30fps — cuts heard landing just after their beat). Pixels are identical; the container timing is not, so cached clips are invalidated to purge the lag. |
| v19 | `brightness` maps its centroid **logarithmically** across the band (was linear). Pitch is logarithmic and so is where musical energy lives: a real centroid (~1–2 kHz) landed at 5% of a full 20 Hz–22 kHz band and moved by 4%, so the curve sat on the floor no matter what the music did. Measured full-band, the mean goes 0.05 → 0.63 and a dull stem's swing 0.043 → 0.122; lively material gives back ~20% because log compresses the top. Band edges still map to exactly 0 and 1. |
