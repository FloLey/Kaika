...
| v14 | A video card out of material with `loop` off renders **blank** for the rest of its window (was: hold the last frame). A montage slot longer than its clip froze on a still for the whole cut; blank is unambiguous, and the card's shortfall warning already reports the deficit. New video cards also default to `loop: false`. |
